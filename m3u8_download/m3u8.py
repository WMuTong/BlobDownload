...
def read_concat_list(concat_list_filen_path):
    if os.path.exists(concat_list_filen_path):
        with open(concat_list_filen_path, "r") as concat_file:
            return [
                line.strip().split("file '")[1].rstrip("'")
                for line in concat_file.readlines()
            ]
    return []
...
# 选择 M3U8 文件
m3u8_path = filedialog.askopenfilename(
    title="选择 M3U8 文件", filetypes=[("M3U8 文件", "*.m3u8")]
)
...
pattern = r"/hjstore/video/([^/]+)/([^/]+)/([^/]+\.ts)"
matches = re.findall(pattern, m3u8_content)
ts_urls = [
    f"https://ts3.hjbd81.top/hjstore/video/{part1}/{part2}/{part3}"
...
for i, ts_url in enumerate(ts_urls):
    ...
    # 将解密后的TS文件路径写入合并列表文件
    write_concat_list_entry(
        os.path.join("../", decrypted_file_path), concat_list_filen_path
    )
...
if result.returncode != 0:
    ...
    for filePath in existing_files:
        os.remove(
            os.path.join(
                os.path.abspath(os.path.dirname(__file__)), filePath.replace("../", "")
            )
        )
